//! `bootstrap` — the AWS Lambda entry for the thin Chronos-Bolt MCP server.
//!
//! The loopback-proxy pattern proven by `aprender-mcp-setfit-lambda`: the pmcp
...
//! The model resolves ONCE per container (embedded weights or env path) and
//! stays warm in memory across invocations.
...
const DEFAULT_SERVER_NAME: &str = "chronos-forecaster";
...
fn build_server() -> pmcp::Result<pmcp::Server> {
    ...
    aprender_mcp_chronos_lambda::build_server(model, &name, env!("CARGO_PKG_VERSION"))
}
...
async fn start_http_in_background(default_port: u16) -> pmcp::Result<SocketAddr> {
    ...
    let http_server = pmcp::server::streamable_http_server::StreamableHttpServer::with_config(
        addr,
        server,
        aprender_mcp_chronos_lambda::server_config(),
    );
    let (bound, handle) = http_server.start().await?;
    let name = server_name();
    tracing::info!("{name}: MCP server started on {bound}");
...
async fn handler(event: Request) -> Result<Response<Body>, Error> {
    ...
    if method.as_str() == "GET" {
        ...
        let body = serde_json::json!({
            "ok": true,
            "server": name,
            "message": "Chronos-Bolt zero-shot forecasting MCP server. POST JSON-RPC to '/' for MCP requests."
        })
        .to_string();
